...
import 'day_button_active.dart';
import 'day_button_inactive.dart';

List mesSelect(type) {
  Color colorCont = const Color.fromRGBO(0, 0, 0, 0);
  Color cor = const Color.fromRGBO(0, 0, 0, 0);
  switch (type) {
    case 1: //teleconsulta
      {
        cor = const Color.fromRGBO(255, 255, 255, 1);
        colorCont = const Color.fromRGBO(137, 169, 255, 1);
      }
      break;
    case 2: //Normal
      {
        cor = const Color.fromRGBO(255, 255, 255, 1);
        colorCont = const Color.fromRGBO(183, 67, 52, 1);
      }
      break;
    case 3: //a aguardar
      {
        cor = const Color.fromRGBO(255, 255, 255, 1);
        colorCont = const Color.fromRGBO(180, 180, 180, 1);
      }
      break;
  }
...
class CalendarDayButtonGroup extends StatefulWidget {
  ...
  const CalendarDayButtonGroup(this.selectedMonth, this.selectedYear, this.notifyParent, {Key? key}) : super(key: key);
...
class _CalendarDayButtonGroupState extends State<CalendarDayButtonGroup> {
  ...
  Widget build(BuildContext context) {
    ...
    //Cores dos dias (WIP)
    Color cor = const Color.fromRGBO(137, 169, 255, 1);
    FontWeight font = FontWeight.w600;
    Color colorCont = Colors.transparent;
    EdgeInsets small = const EdgeInsets.fromLTRB(5, 5, 5, 5);

    //MES ATUAL FORMULAS
    int amountOfDaysThisMonth = DateTime(widget.selectedYear, widget.selectedMonth + 1, 0, 0, 0, 0, 0).day; //numero de dias
    DateTime thisMonthFirstDay = DateTime(widget.selectedYear, widget.selectedMonth, 1, 0, 0, 0, 0); //obter primeiro

    //MES ANTERIOR FORMULAS
    DateTime monthBeforeStartDay = thisMonthFirstDay.subtract(Duration(days: thisMonthFirstDay.weekday)).add(const Duration(hours: 1)); // primeiro dia da semana a mostrar
    ...
    for (var dia in arrayDiasSemana) {
